...
def main():
  ...
  print("   st_all =", np.round(np.sum(t.astype(np.float32), axis=0).astype(np.float16) / 1000.0, 2).astype(np.float16))
  ...
  for i in range(10):
    print(f" s_bin({i}) =", binary(use_stochastic_rounding=1)(c).numpy().astype(np.int32))
  for i in range(10):
    print(f" s_po2({i}) =", quantized_po2(use_stochastic_rounding=1)(c).numpy().astype(np.int32))
  for i in range(10):
    print(f" s_relu_po2({i}) =", quantized_relu_po2(use_stochastic_rounding=1)(c).numpy().astype(np.int32))
