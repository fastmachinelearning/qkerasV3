...
class ForgivingFactorBits(ForgivingFactor):
    ...
    def compute_model_size(self, model):
        ...
        for layer in model.layers:
            ...
            layer_config = self.config.get(layer_name, self.config.get("default", None))
            if layer_config:
    ...
    def print_stats(self):
        ...
        print(
                ...
                f"       a_bits={int(self.total_a_bits)}/{int(self.ref_a)} ({a_percentage:.2f}%) p_bits={int(self.total_p_bits)}/{int(self.ref_p)} ({p_percentage:.2f}%)\n"
                f"       total={int(trial_total)}/{int(ref_total)} ({total_percentage:.2f}%)"
