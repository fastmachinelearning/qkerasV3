...
from qkeras.utils import clone_model, clone_optimizer, model_quantize
...
class AutoQKHyperModel(HyperModel):
        ...
        for name in REGISTERED_LAYERS:
            if name in self.limit:
                ...
                if length < 4 and name in SEQUENCE_LAYERS:
                    ...
                    self.limit[name] = (
                        self.limit[name] + default[length:2] + default[-1:]
                    )
    ...
    def _get_quantizer(
        ...
        if isinstance(self.limit[name][index], list):
            ...
            q_dict = {key: q_dict[key] for key in q_list}
        else:
            q_dict = {
                key: value
    ...
    def quantize_model(self, hp):
        ...
        for layer in model.layers:
            if layer.__class__.__name__ in REGISTERED_LAYERS:
                ...
                if kernel_quantizer:
                    if (
                        not filter_sweep_enabled
                        and self.tune_filters in ["layer", "block"]
                        and not self.tune_filters_exceptions.search(layer.name)
                        and layer.__class__.__name__
                        in ["Dense", "Conv1D", "Conv2D", "Conv2DTranspose"]
                    ):
        ...
        if self.tune_filters == "block" and filter_sweep_enabled:
            network_filters = hp.Choice(
                "network_filters", values=filter_range, default=1.0
            )
        else:
            network_filters = 1.0
        ...
        for layer_id, layer in enumerate(model.layers):
            ...
            if layer.__class__.__name__ in Q_LAYERS:
                ...
                if layer.get_quantizers()[0] and hasattr(
                    layer.get_quantizers()[0], "bits"
                ):
            ...
            if layer.__class__.__name__ in REGISTERED_LAYERS:
                ...
                if (
                    self.tune_filters in ["layer", "block"]
                    and not self.tune_filters_exceptions.search(layer.name)
                    and layer.__class__.__name__
                    in [
                        "Dense",
                ...
                if layer.__class__.__name__ == "Bidirectional":
                    ...
                    if layer.activation is None:
                        is_softmax = False
                        is_linear = False
                    elif isinstance(layer.activation, six.string_types):
                ...
                if (
                    self.tune_filters == "none"
                    or layer.name not in self.limit
                ...
                if K.image_data_format() == "channels_last":
                    ...
                    layer.target_shape = (
                        int(layer.target_shape[0] * network_filters),
...
class AdjustedScore(keras.metrics.Metric):
    def __init__(
        ...
        self.result_tracker = self.add_weight(
            name="adjusted_score_value", initializer="zeros"
        )
    ...
    def get_config(self):
        ...
        config.update({"delta": self.delta, "metric_name": self.metric_name})
        return config
...
class AutoQKeras:
    ...
    def history(self, number_of_trials=-1):
        ...
        result = {}
        result["score"] = [
            state[i]["score"] for i in range(len(state)) if trials[i].score is not None
        ]
...
class AutoQKerasScheduler:
    ...
    def fit(self, *fit_args, **fit_kwargs):
        ...
        for i, (pattern, cost) in enumerate(self.retrieve_max_block()):
            ...
            print(f"... block cost: {cost:.0f} / {self.reference_size:.0f}")
            ...
            if self.tuner_kwargs.get("max_trials", None):
                ...
                print(
                    "... adjusting max_trials for this block to {}".format(
            ...
            with open(os.path.join(self.output_dir, "scheduler.json"), "w") as f:
                json.dump({"next_block": self.next_block}, f)
