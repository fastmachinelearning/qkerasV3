...
def get_model():
    ...
    x = QConv2D(
        ...
        bias_quantizer="binary",  # pylint: disable=undefined-variable
        strides=1,
        padding="same",
        name="c2",
    )(x)
    ...
    x = QConv2D(
        ...
        bias_quantizer="ternary",  # pylint: disable=undefined-variable
        strides=1,
        padding="same",
        activation="binary",
        name="c3",
...
def test_forgiving_factor_bits():
    ...
    ffb = ForgivingFactorBits(
        delta_p, delta_n, rate, stress, input_bits, output_bits, ref_bits, config
    )
...
def test_new_forgiving_factor():
    ...
    q_dict = {
        "c1": {"kernel_quantizer": "binary", "bias_quantizer": "quantized_bits(4)"}
    }
