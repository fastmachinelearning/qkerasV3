...
def print_qstats(graph):
    ...
    for layer, data_type in layer_data_type_map.items():
        ...
        print(
            f"{layer.name}: {data_type.operation_count} x {multiplier_detail_str}"
        )

        multipliers_counter[multiplier_detail_str] += data_type.operation_count
    ...
    ) in multipliers_counter.items():
        print(
            f"{multiplier_detail_str}, x {total_multiplier_operation_count}"
        )
...
def map_to_json(mydict):
    ...
    for layer, feature in layer_data_type_map.items():
        ...
        layer_item["input_quantizer_list"] = [
            ...
            for q in qtools_util.get_val(feature, "input_quantizer_list")
        ]
        ...
        if layer_item["layer_type"] in ["QBatchNormalization", "BatchNormalization"]:
            for key in [
                "gamma_quantizer",
                "beta_quantizer",
                "mean_quantizer",
            ...
            for key in [
                ...
                set_layer_item(
                    layer_item, key=key, feature=feature, is_compound_datatype=True
                )
        ...
        layer_item["operation_count"] = qtools_util.get_val(feature, "operation_count")
