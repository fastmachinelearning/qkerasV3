...
class QDepthwiseConv2D(layers.DepthwiseConv2D, PrunableLayer):
    ...
    def get_config(self):
        ...
        config["depthwise_quantizer"] = serialize_keras_object(
            self.depthwise_quantizer_internal
        )
    ...
    def get_quantization_config(self):
        return {
            "depthwise_quantizer_internal": str(self.depthwise_quantizer_internal),
            "bias_quantizer": str(self.bias_quantizer_internal),
            "activation": str(self.activation),
            "filters": str(self.filters),
        }
    ...
    def _call(inputs):  # pylint: disable=invalid-name
        ...
        if pw_first:
            ...
            if depthwise_dropout_rate > 0.0:
                x = layers.Dropout(
                    rate=depthwise_dropout_rate, name=name + "_dw_dropout"
                )(x)
