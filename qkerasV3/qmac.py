...
# qkerasV3 needs to support more layers for matrix multiplication and shift
...
class QScaleShift(keras.layers.Layer, PrunableLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        _, self.weight_initializer = get_auto_range_constraint_initializer(
            self.weight_quantizer_internal, None, weight_initializer
        )

        _, self.bias_initializer = get_auto_range_constraint_initializer(
        ...
        self.quantizers = [self.weight_quantizer_internal, self.bias_quantizer_internal]
    ...
    def call(self, inputs):
        quantized_weight = (
            self.weight_quantizer_internal(self.weight)
            if self.weight_quantizer_internal is not None
            else self.weight
        )
        ...
        if self.use_bias:
            quantized_bias = (
                self.bias_quantizer_internal(self.bias)
    ...
    def get_quantization_config(self):
        return {
            "weight_quantizer": str(self.weight_quantizer_internal),
