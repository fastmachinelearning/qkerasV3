...
def _get_msqe_scale(x, q, scale_axis=None, per_channel_scale=True, msqe_weight=None):
...
class BaseQuantizerPO2(Layer):  # pylint: disable=invalid-name
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.scaled_axes = (
            kwargs.pop("scaled_axes") if "scaled_axes" in kwargs else None
        )
    ...
    def build(self, input_shape):
        ...
        if self.init_scale is None:
            ...
            init_scale_exponent = Kops.log(init_scale) / Kops.log(2.0)
        ...
        self.reduce_axes = [
            i
            for i in range(len(self.scale_exponent.shape))
    ...
    def _get_msqe_weight(self, inputs=None):
        ...
        if self.use_outlier_mask_msqe_weight and inputs is None:
            raise ValueError(
                "inputs must not be None if self.use_outlier_mask_msqe_weight is"
                " True."
            )
    ...
    def _get_stable_scale(self, scale):
        ...
        should_update = Kops.logical_and(
            self.should_update_stable_scale_exponent, not self.switch_to_stable_scale
        )
        ...
        stable_scale = self._update_stable_scale_exponent(
    ...
    def _clip_quant(self, inputs):
        ...
        """
        return Kops.minimum(Kops.maximum(inputs, -self.qn), self.qp)
    ...
    def _calculate_msqe_inputs(self, inputs, scale, reduce_axes=None, msqe_weight=None):
...
class quantized_bits_learnable_po2(BaseQuantizerPO2):  # pylint: disable=invalid-name
    ...
    def __str__(self):
        ...
        flags.append(
            "use_second_moments_msqe_opt=" + str(self.use_second_moments_msqe_opt)
        )
        flags.append("second_moments_ema_decay=" + str(self.second_moments_ema_decay))
        flags.append(
            "use_outlier_mask_msqe_weight=" + str(self.use_outlier_mask_msqe_weight)
        )
        flags.append("use_sqrt_of_msqe_weight=" + str(self.use_sqrt_of_msqe_weight))
        flags.append("use_stable_scale_exponent=" + str(self.use_stable_scale_exponent))
        ...
        flags.append("use_po2_scale_msqe_round=" + str(self.use_po2_scale_msqe_round))
