                ...
                for k, kernel in enumerate(kernels):
                    ...
                    if self.use_separable == "mobilenet":
                        x = DepthwiseConv2D(
                            ...
                            strides=strides,
                            use_bias=False,
                            name="conv2d_dw_" + depth_suffix,
                        )(x)
                        if self.with_batchnorm:
                            x = BatchNormalization(name="conv2d_dw_bn_" + depth_suffix)(
                                x
                            )
                    ...
                    x = Conv2D(
                        ...
                        kernel,
                        strides=strides,
                        use_bias=not self.with_batchnorm,
                        padding="same",
                        kernel_initializer=self.kernel_initializer,
                        bias_initializer=self.bias_initializer,
                        name="conv2d_" + kernel_suffix,
                    )(x)
                    if not (
                        self.with_maxpooling
                        and self.use_xnornet_trick
                        and j == len(filters) - 1
                        and k == len(kernels) - 1
                    ):
                        if self.with_batchnorm:
