...
class QTools:
        ...
        def _get_ace(layer):
            ...
            if layer.name in self._output_dict:
                ...
                if weight_quantizer:
                    ...
                    ace_float = (
                        operation_count * default_float_bits * default_float_bits
                    )
            return (ace, ace_float)

        print("WARNING: ACE are computed from conv/dense layers only!")
        return (
            sum([_get_ace(l)[0] for l in self._model.layers]),
            sum([_get_ace(l)[1] for l in self._model.layers]),
        )

    def calculate_output_bytes(self, include_model_input_size, default_float_bits):
        ...
        output_bytes_float = sum(
            [_get_activation_size(l)[1] for l in self._model.layers]
        )
    ...
    def calculate_weight_bytes(self, default_float_bits):
            ...
            if layer.name in self._output_dict:
                ...
                if weight_quantizer:
                    ...
                    if hasattr(layer, "use_bias") and layer.use_bias:
                        bias_quantizer = qtools_util.get_val(
                            layer_item, "bias_quantizer"
                        )
        ...
        return (
            sum([_get_weight_size(l)[0] for l in self._model.layers]),
            sum([_get_weight_size(l)[1] for l in self._model.layers]),
        )

    def get_roofline_numbers(
        self, include_model_input_size=True, default_float_bits=32
    ):
        """Extracts model numbers for roofline model analysis."""

        return {
            "ACE": self.calculate_ace(default_float_bits)[0],
            "weight_in_bytes": self.calculate_weight_bytes(default_float_bits)[0],
            "activation_in_bytes": self.calculate_output_bytes(
                include_model_input_size, default_float_bits
            )[0],
            "ACE_float": self.calculate_ace(default_float_bits)[1],
