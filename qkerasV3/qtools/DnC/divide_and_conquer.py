    ...
    def get_layer_quantizer_bitwidth(self, node: int | tf.keras.layers.Layer):
        ...
        if layer:
            ...
            return {
                ...
                "mac_bits": (
                    mac_quantizer.output.bits
                    if mac_quantizer
                    else input_quantizer_list[0].bits
                ),
                "acc_bits": (
                    acc_quantizer.output.bits
                    if acc_quantizer
                    else input_quantizer_list[0].bits
                ),
...
class Choice:
    ...
    def __str__(self):
        return (
            f"Choice(k={self.k}, l={self.l}, cin_unroll={self.cin_unroll}, "
    ...
    for cin_unroll in cin_unroll_list:
        for kw_unroll in kw_unroll_list:
            for kh_unroll in kh_unroll_list:
                logging.debug(
                    "............cin_unroll: %d kh_unroll: %d kw_unroll: %d",
                    cin_unroll,
                    kh_unroll,
                    kw_unroll,
                )
...
def get_per_layer_cost(
    layer_quantizer_bitwidth,
    layer_mac_count,
    layer_shapes,
    cin_unroll,
    cout_unroll,
    ...
    memory_area = (
        InElementPerClk
        * layer_quantizer_bitwidth["input_bits"]
        * dnc_layer_cost_ace.MemoryGatesPerBit["Register"]
        + OutElementPerClk
        * layer_quantizer_bitwidth["output_bits"]
        * dnc_layer_cost_ace.MemoryGatesPerBit["Register"]
...
def get_valid_candidates(input_value, output_to_input_ratio_max):
    ...
    if output_to_input_ratio_max >= 2:
        candidate_list += [
            input_value * x for x in list(range(2, output_to_input_ratio_max + 1))
        ]
...
def is_bufferThru_greater_than_targetThru(
    ...
    return (
        InBuf_throughput >= target_out_throughput
        and OutBuf_throughput >= target_in_throughput
    )


def set_best_global_cost_in_paths(
    OutElementPerClk_list,
    paths,
    layer_idx,
    cur_layer_idx,
    layer_quantizer_bitwidth,
    ...
    def calculate_cost(OutElementPerClk):
        ...
        accumulative_cost = (
            cur_layer_cost + paths[layer_idx][OutElementPerClk]["acc_cost"]
        )
        return (cur_layer_cost, accumulative_cost, OutElementPerClk)

    cost_and_values = list(map(calculate_cost, OutElementPerClk_list))

    layer_cost, min_accumulative_cost, best_OutElementPerClk = min(
        cost_and_values, key=lambda x: x[1]
    )
...
def update_cur_best_choices(
    ...
    logging.debug(
        "...............cost of cur_best_choices [%d]: %.2f",
        prev_OutElementPerClk,
        existing_accumulative_cost,
    )
...
def get_ComputeInElementPerClk(
...
def get_pe_throughput(
    ...
    elif layer_type in ["QGlobalAveragePooling2D", "GlobalMaxPooling2D", "Concatenate"]:
...
def calc_hw_params(
    ...
    while graph.get_prev_nodes(layer_idx):
        ...
        logging.debug(
            "processing layer_idx:%d name:%s type:%s ***",
            cur_layer_idx,
            getattr(cur_layer, "name", None),
        ...
        logging.debug(
            "input_channel: %d, output_channel: %d, kernel_height: %d, "
        ...
        for OutElementPerClk in OutElementPerClk_list:
            ...
            if layer_type in [
                ...
                logging.debug(
                    "...... Passing through layer_type: %s with 0 cost", layer_type
                )
                ...
                update_cur_best_choices(
                    cur_best_choices,
                    OutElementPerClk=OutElementPerClk,
                    prev_OutElementPerClk=OutElementPerClk,
                    cur_layer_cost=0,
            ...
            for ComputeOutElementPerClk in get_valid_candidates(
                ...
                logging.debug(
                    "......ComputeOutElementPerClk: %d", ComputeOutElementPerClk
                )
                ...
                valid_unrolls = get_valid_unrolls(
                    cur_layer, cout_unroll, target_pe_throughput
                )
                ...
                for cin_unroll, kh_unroll, kw_unroll in valid_unrolls:
                    ...
                    ComputInElementPerClk = get_ComputeInElementPerClk(
                        layer_type,
                        cin_unroll=cin_unroll,
                        cout_unroll=cout_unroll,
                        kh_unroll=kh_unroll,
                        kw_unroll=kw_unroll,
                    )
                    ...
                    InElementPerClk_base = get_InElementPerClk_base(
                        ComputInElementPerClk=ComputInElementPerClk,
                        kh_unroll=kh_unroll,
                        kw_unroll=kw_unroll,
                    )
                    for InElementPerClk in get_valid_candidates(
                        ...
                        logging.debug(
                            "............InElementPerClk: %d", InElementPerClk
                        )
                        ...
                        if is_bufferThru_greater_than_targetThru(
                            layer_type=layer_type,
                            InElementPerClk=InElementPerClk,
                            OutElementPerClk=OutElementPerClk,
                            ...
                            accumulative_cost = (
                                cur_layer_cost
                                + paths[layer_idx][OutElementPerClk]["acc_cost"]
                            )

                            logging.debug(
                                "...............Buf throughput is good! "
                                "Accumulative_cost: %.2f",
                                accumulative_cost,
                            )

                            # Each choice is a hw param combination.
                            choice = Choice(
                                l, k, cin_unroll, cout_unroll, kh_unroll, kw_unroll
                            )
        ...
        if not cur_best_choices:
            logging.error(
                "Cannot find any valid HW choice for layer %s! Exit!", cur_layer.name
            )
            return {}, None
...
def estimate_model_cost(
    ...
    best_path, best_cost = calc_hw_params(
        graph=graph,
        target_OutElementPerClk=target_OutElementPerClk,
        target_out_throughput=target_out_throughput,
        input_quantizer_bits=input_quantizer_bits,
        compute_to_memory_max_ratio=(compute_to_memory_max_ratio),
        memory_to_unroll_max_ratio=(memory_to_unroll_max_ratio),
        mode=mode,
    )
