...
def get_operation_count(layer, input_shape):
    ...
    if is_merge_layers(layer) or is_shape_alternation_layers(layer):
        ...
        operation_count = time_o * channels_o * kernel_length * channels_i
        ...
        operation_count = kernel_h * kernel_w * height_o * width_o * channels_i
        ...
        print(f"operation count for {layer} is defaulted to 0")
    ...
    if not model_weights_already_quantized:
        for j, weight in enumerate(weights):
            if hasattr(layer, "get_quantizers") and layer.get_quantizers()[j]:
                out[j] = K.eval(layer.get_quantizers()[j](K.constant(weight)))
    return out
    ...
    if (
        ...
        if scale is not None:
            if isinstance(scale, KerasTensor) or isinstance(scale, np.ndarray):
                ...
                raise ValueError(
                    f"Scale should be either numpy array or float,"
            ...
            print(
                "[WARNING] The weight quantizer is never called even though it has "
...
def adjust_accumulator_for_auto_po2(
    ...
    if layer.__class__.__name__ in ["QDepthwiseConv2D", "DepthwiseConv2D"]:
        assert kernel_shape[-1] == 1, (
            "depth_multiplier must be 1, " f"{kernel_shape[-1]} found instead!"
        )
...
def get_layer_info(layer: keras.layers.Layer, attr_name: str):
    ...
    kernel_height, kernel_width = (
        layer.kernel_size if hasattr(layer, "kernel_size") else (1, 1)
    )
