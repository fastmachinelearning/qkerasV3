...
def test_convert_to_folded_model():
    ...
    def get_add_model():
        ...
        x1 = layers.Conv2D(
            4, kernel_size=(2, 2), padding="valid", strides=(1, 1), name="conv2d_1"
        )(x)
        x1 = layers.BatchNormalization(name="bn_1")(x1)
        x1 = layers.Activation("relu", name="relu_1")(x1)
        x2 = layers.Conv2D(
            4, kernel_size=(2, 2), padding="valid", strides=(1, 1), name="conv2d_2"
        )(x)
...
def test_find_bn_fusing_layer_pair():
    ...
    x1 = QBatchNormalization(
        ...
        inverse_quantizer=quantized_bits(8, 0, 1),
        name="bn1",
    )(x1)
...
def test_clone_model_and_freeze_auto_po2_scale_error():
    ...
    with pytest.raises(ValueError):
        clone_model_and_freeze_auto_po2_scale(orig_model, quantize_model_weights=False)
