...
def pooling_qmodel():
    ...
    x = keras.layers.AveragePooling2D(
        pool_size=(2, 2), padding="valid", name="avg_pooling"
    )(x)
...
def run(
    model, input_quantizers, is_inference=False, verbose=False, hw_weight_dict=None
):
    (graph, source_quantizer_list) = qgraph.CreateGraph(model, input_quantizers)
...
def test_qbn_inference():
    ...
    dtype_dict = run(
        model, input_quantizers, is_inference=True, hw_weight_dict=hw_weight_dict
    )
...
def test_invalid_denominator_qbn():
    ...
    variance = gamma
    model = qbn_model(act=act, gamma=gamma, variance=variance, beta=None, mean=None)
...
def test_conv2d():
    ...
    x = QActivation(act, name="QA_0")(x)
    x = QConv2D(
        16, 2, 2, kernel_quantizer=weight, bias_quantizer=weight, name="qconv2d_1"
    )(x)
...
def test_quntized_reference_energy_same_as_floating_trial():
    ...
    _, float_trial_energy = get_qenergy(float_model, qenergy_config, for_reference=True)
...
def test_auto_po2():
    def gen_model(img_shape):
        ...
        x = QDense(
            5,
            kernel_quantizer=quantizers.quantized_bits(8, 0, alpha="auto_po2"),
            bias_quantizer=quantizers.quantized_bits(8, 3),
            name="dense",
        )(x)
