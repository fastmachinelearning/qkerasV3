...
def file_compress(fin, fout):
    ...
    f.close()
    print(
        f"... random forrest for {os.path.basename(fin)} reduced from {n_lines} to {n_compressed} entries"
    )
...
def mp_rf_optimizer_func(fn_tuple):
    ...
    print(f"... saving model in {rfb_file}")
...
def mp_abc_optimizer_func(fn):
    ...
    cmd = f"espresso {fn} {espresso_flags} > {o_file}"
...
def run_abc_optimizer(files):
    ...
    pool.close()
    print(f"Optimizer ran in {time.time() - start_time} seconds.")
...
def run_rf_optimizer(files, flags, file_suffix="cc"):
    ...
    pool = mp.Pool(cpus)
    pool.map(
        mp_rf_optimizer_func,
        zip(files, [flags] * len(files), [file_suffix] * len(files)),
    )
    pool.close()
    print(f"Optimizer ran in {time.time() - start_time} seconds.")
