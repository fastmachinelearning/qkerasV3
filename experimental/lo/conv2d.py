...
def parallel_index_table(
    ...
    print(
        f"... indexing table from {ni} to {ni + size} ({p[0].shape} => {p[1].shape})"
    )
                ...
                if table_set.has_entry(table_s) and not is_table_zero:
                    ...
                    if DEBUG:
                        ...
                        if table_o != table_o_old:
                            print(
                                f"contradicting outputs n={(n, ho, wo)} old_n={(old_n, old_ho, old_wo)} out_p={table_o_old} out={table_o}"
                            )
                            print(" I:", table_s)
                            print(" I:", i_values)
                            print("<<<", table_o_old)
    ...
    print(f"... indexing table from {ni} to {ni + size} completed")
...
def optimize_conv2d_logic(
    ...
    if randomize:
        idx_height = np.random.choice(p[-1].shape[1], int(knp.round(knp.sqrt(randomize))))

        idx_width = np.random.choice(p[-1].shape[2], int(knp.round(knp.sqrt(randomize))))
    else:
    ...
    print(f"... generating tables {o_size} outputs, {len(table_ins)} entries")
    ...
    for o in range(0, o_size, output_group):
        filename = f"{prefix}{i_name}.{suffix}/{i_name}_{o}.raw.{suffix}"
        ...
        if generate_pla:
            header.append(f".i {ni + n_bits_og}\n")
            header.append(f".o {no // o_size}\n")
            header.append(".type fr\n")

            if OG_IS_SYMBOLIC and output_group > 1:
                header.append(
                    f".mv {3} {ni} {n_bits_og} {no // o_size}\n"
                )
            ...
            header.append(
                ".ob "
                + " ".join(
                    ["o_" + str(o) + "_" + str(o_bits - 1 - v) for v in range(o_bits)]
                )
                + "\n"
            )
