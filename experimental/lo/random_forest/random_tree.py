...
class RandomTree:
    ...
    def select_best_split(self, dataset):
        ...
        features = list(
            np.random.choice(
                len(dataset[0]) - 1, self.n_features, p=self.probs, replace=False
            )
        )
    ...
    def split_node(self, node, depth):
        ...
        if depth >= self.max_depth:
            node["left"], node["right"] = (
                self.select_terminal(left),
                self.select_terminal(right),
            )
            return

        # process left child
        if len(set(list(left[:, -1].flatten()))) == 1 or left.shape[0] <= self.min_size:
        ...
        # process right child
        if (
            len(set(list(right[:, -1].flatten()))) == 1
            or right.shape[0] <= self.min_size
        ):
