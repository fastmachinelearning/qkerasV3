...
def parallel_index_table(p, ni, size, i_dict, o_dict, generate_pla):
        ...
        if table_set.has_entry(table_s) and not is_table_zero:
            ...
            if DEBUG:
                ...
                if table_o != table_o_old:
                    print(
                        f"contradicting outputs n={n} old_n={old_n} out_p={table_o_old} out={table_o}"
                    )
                    print(" I:", table_s)
                    print(" I:", i_values)
                    print("<<<", table_o_old)
        ...
        if generate_pla:
            ...
            for feature in range(i_size):
                for bit in range(i_bits):
                    i_names.append(f"i_{feature}_{i_bits - 1 - bit}")
